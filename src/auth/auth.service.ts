...
export class AuthService {
  ...
  async validateOAuthLogin(
    ...
    if (!user) {
      user = this.userRepo.create({
        email,
        first_name:
          profile.name?.givenName || profile.name?.firstName || 'OAuth',
  ...
  async forgotPassword(forgotPasswordDto: ForgotPasswordDto) {
    ...
    if (!user)
      return {
        message:
          'If that email exists, a reset link has been sent to that email',
      };
  ...
  async resetPassword(resetPasswordDto: ResetpasswordDto) {
    ...
    // clear token immediately (important security step)
