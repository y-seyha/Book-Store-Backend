...
export class ProductsService {
  ...
  async findAll(query: QueryProductDto): Promise<ProductListResponse> {
    ...
    if (cached) return cached as ProductListResponse;
    ...
    const sortBy = ['price', 'name', 'created_at'].includes(
      normalizedQuery.sortBy,
    )
      ? normalizedQuery.sortBy
      : 'created_at';

    qb.orderBy(`product.${sortBy}`, normalizedQuery.order);
  ...
  async update(id: number, updateDto: UpdateProductDto) {
    ...
    if (updateDto.image_url !== undefined)
      product.image_url = updateDto.image_url;
    if (updateDto.image_public_id !== undefined)
      product.image_public_id = updateDto.image_public_id;
